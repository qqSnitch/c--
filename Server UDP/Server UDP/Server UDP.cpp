...
int main() {
    ...
    {
        cout << "WSAStartup error: " << WSAGetLastError();//ошибка
        return -1;
    }
    // шаг 2 - создание сокета
    SOCKET Lsock;// создаем сокет
    Lsock = socket(AF_INET, SOCK_DGRAM, 0);//это протокол на основе дейтаграмм. Вы отправляете одну дейтаграмму и получаете один ответ, после чего соединение прерывается.
    if (Lsock == INVALID_SOCKET) {
        cout << "SOCKET() ERROR: " << WSAGetLastError();//ошибка в сокете
        WSACleanup();
        return -1;
    }
    // шаг 3 - связывание сокета с локальным адресом 
    sockaddr_in Laddr;// создание и заполнение структуры адреса клиента
